...
const saveSituations = (situations) => {
    fs.writeFileSync(situationsPath, `${JSON.stringify({ situations }, null, 2)}\n`, 'utf-8');
};
const syncCatalogAcrossRooms = (catalog) => {
...
const validateSituationPayload = (payload) => {
    if (!payload || typeof payload !== 'object') {
        return { valid: false, error: 'Situación inválida.' };
    }
    const { id, title, description, options } = payload;
...
const resolveConfiguredSituations = (room, randomize) => {
    ...
    const ordered = room.configuredSituationIds
        ...
        .filter((situation) => Boolean(situation));
    return randomize ? shuffleArray(ordered) : ordered;
};
const getRoomOr404 = (code, res) => {
...
const getCodeFromParams = (codeParam) => {
    if (!codeParam)
        return null;
    return Array.isArray(codeParam) ? (codeParam[0] ?? null) : codeParam;
};
const validateAdmin = (room, adminId, res) => {
...
const getRoomData = (room, viewerPlayerId, includeChoicesByPlayer = false) => {
...
io.on('connection', (socket) => {
    socket.on('join_network_room', (payload) => {
    ...
    socket.on('disconnect', () => {
        ...
        const timer = setTimeout(() => {
            ...
            if (!latestRoom)
                return;
            if (latestRoom.connectedPlayerIds.has(playerId))
                return; // reconnected
...
app.put('/api/situations/:id', (req, res) => {
...
app.post('/api/rooms/join', (req, res) => {
    ...
    const newPlayer = { id: crypto.randomUUID(), username, role };
...
app.post('/api/rooms/:code/start', (req, res) => {
    ...
    if (!room)
        return;
    if (!validateAdmin(room, adminId, res))
        return;
    if (room.status !== 'LOBBY') {
...
app.post('/api/rooms/:code/unvote', (req, res) => {
    const code = getCodeFromParams(req.params.code);
...
app.post('/api/rooms/:code/admin/configure-situations', (req, res) => {
    const code = getCodeFromParams(req.params.code);
    const { adminId, configuredSituationIds, randomizeSituations } = req.body;
...
app.post('/api/rooms/:code/admin/add-situation', (req, res) => {
    const code = getCodeFromParams(req.params.code);
    const { adminId, situation } = req.body;
    if (!code) {
        res.status(400).json({ error: 'Código de sala inválido.' });
        return;
    }
...
app.post('/api/rooms/:code/admin/edit-choice', (req, res) => {
    const code = getCodeFromParams(req.params.code);
    const { adminId, targetPlayerId, optionId } = req.body;
...
app.post('/api/rooms/:code/admin/confirm-votes', (req, res) => {
    const code = getCodeFromParams(req.params.code);
    const { adminId } = req.body;
    if (!code) {
        res.status(400).json({ error: 'Código de sala inválido.' });
...
app.post('/api/rooms/:code/admin/anticipation', (req, res) => {
    const code = getCodeFromParams(req.params.code);
    const { adminId, predictedOptionId } = req.body;
...
app.post('/api/rooms/:code/admin/apply-consequence', (req, res) => {
    const code = getCodeFromParams(req.params.code);
    const { adminId } = req.body;
    if (!code) {
        res.status(400).json({ error: 'Código de sala inválido.' });
...
app.post('/api/rooms/:code/admin/next', (req, res) => {
    const code = getCodeFromParams(req.params.code);
    const { adminId } = req.body;
    if (!code) {
        res.status(400).json({ error: 'Código de sala inválido.' });
...
app.get('/api/admin/finished-games', (req, res) => {
...
app.get('/api/admin/finished-games/:gameId/analytics', (req, res) => {
...
app.get('/api/admin/finished-games/:gameId/report.pdf', async (req, res) => {
    const gameId = getCodeFromParams(req.params.gameId);
    const adminId = String(req.query.adminId ?? '');
    if (!gameId || !adminId) {
        res.status(400).json({ error: 'Parámetros inválidos.' });
